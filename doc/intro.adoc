...
The easiest way to do this is to use the `select_biased!` macro from the
`futures` crate. (It's called `biased` because it steps the state machines
inside it from first to last, and if any complete, all the rest are dropped.
This means it's slightly _biased_ toward completing the earlier ones.)
